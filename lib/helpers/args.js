...
let consoleWarning = (paramType, paramName, fallbackVal) => {
  console.log(
    `${colors.yellow('Warning')}: No ${paramType} set with param -${paramName}, using fallback "${fallbackVal}".`
  )
}

let consoleError = (paramType, paramName) => {
  console.log(
    `${colors.red('Error')}: No ${paramType} set with param -${paramName}, cannot continue, exiting.".`
  )
  process.exit()
}
